...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    // Maintain 0° angle when button A is pressed (trigger)
    buttonA.whileTrue(
        DriveCommands.joystickDriveAtAngle(
            drive, () -> -joystick.getY(), () -> -joystick.getX(), () -> new Rotation2d()));

    // Switch to X-pattern when button X is pressed
    buttonX.onTrue(Commands.runOnce(drive::stopWithX, drive));

    // Reset gyroscope when button B is pressed
    buttonB.onTrue(
        Commands.runOnce(
